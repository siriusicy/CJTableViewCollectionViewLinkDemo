...
@interface CJRightCollectionFlowLayout ()

@property (nonatomic, assign) CGFloat naviHeight;
///sectionInsert是否是用代理方法设置
@property (nonatomic, assign) BOOL insetSetWithMethod;
@property (nonatomic, copy) NSArray *decorationAttributes;

@end

@implementation CJRightCollectionFlowLayout
-(instancetype)init
{
    self = [super init];
    if (self)
...
- (void)prepareLayout
{
    ...
    self.insetSetWithMethod = [self.collectionView.delegate respondsToSelector:@selector(collectionView:layout:insetForSectionAtIndex:)];
...
- (void)configureDecorationAttributes
{
    ...
    if (self.sectionBgClassName.length <= 0) {
        return;
    }
    ...
    for (NSInteger section = 0; section < numberOfSection; ++section) {
        ...
        [self registerClass:NSClassFromString(self.sectionBgClassName) forDecorationViewOfKind:self.sectionBgClassName];
        ...
        UICollectionViewLayoutAttributes *attributes = [UICollectionViewLayoutAttributes layoutAttributesForDecorationViewOfKind:self.sectionBgClassName withIndexPath:[NSIndexPath indexPathForRow:0 inSection:section]];
        attributes.zIndex = -1;
        attributes.frame = sectionFrame;
        [decorationAttributes addObject:attributes];
    }
    
    self.decorationAttributes = [decorationAttributes copy];
...
- (CGRect)frameOfSectionViewWithSection:(NSInteger)section
{
    ...
    UIEdgeInsets sectionInset = self.insetSetWithMethod ? [((id<UICollectionViewDelegateFlowLayout>)self.collectionView.delegate) collectionView:self.collectionView layout:self insetForSectionAtIndex:section] : self.sectionInset;
...
- (NSArray *) layoutAttributesForElementsInRect:(CGRect)rect
{
    ...
    //添加section背景信息
    for (UICollectionViewLayoutAttributes *attribute in self.decorationAttributes){
    ...
    for (UICollectionViewLayoutAttributes *attributes in superArray) {
        ...
        UIEdgeInsets sectionInset = self.insetSetWithMethod ? [((id<UICollectionViewDelegateFlowLayout>)self.collectionView.delegate) collectionView:self.collectionView layout:self insetForSectionAtIndex:attributes.indexPath.section] : self.sectionInset;
